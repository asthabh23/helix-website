...
function transformToTableFormat(content) {
  ...
  for (const element of children) {
    if (element.tagName.toLowerCase() === 'br') continue;
  ...
  try {
    ...
    const reportsArrays = await Promise.all(folders.map(async (folder) => {
      ...
      return (await folderResponse.json())
        ...
        .map(item => ({
          filename: item.name.endsWith('.html') ? item.name : `${item.name}.html`,
          path: `${DA_CONFIG.BASE_URL}/${DA_CONFIG.ORG}/${DA_CONFIG.REPO}${DA_CONFIG.UPLOAD_PATH}/${folder}/${item.name.endsWith('.html') ? item.name : item.name + '.html'}`,
